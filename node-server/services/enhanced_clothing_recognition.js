...
class EnhancedClothingRecognitionService {
    constructor() {
    ...
    async recognizeClothing(imagePath) {
        try {
            console.log('开始增强服装识别...');
            
            // 生成缓存键
            const cacheKey = `clothing_${this.generateCacheKey(imagePath)}`;
            ...
            if (!fs.existsSync(imagePath)) {
                ...
                return {
                    ...
                    fallback: {
                        type: '其他',
                        color: { rgb: [128, 128, 128], hex: '#808080', name: '灰色' },
                        style: '休闲',
                        pattern: '纯色',
                        confidence: 0.5
                    }
            ...
            try {
                ...
                console.error('读取图像文件失败:', readError);
                return {
                    success: false,
                    error: '读取图像文件失败',
                    fallback: {
            ...
            // 背景分离
            let clothingOnlyBuffer;
            try {
                clothingOnlyBuffer = await this.separateBackground(imageBuffer);
            } catch (backgroundError) {
                console.error('背景分离失败，使用原始图像:', backgroundError);
                clothingOnlyBuffer = imageBuffer;
            }
            
            // 分析颜色
            let dominantColors, primaryColor;
            try {
                // 尝试使用Python版本的颜色识别
                const pythonColorResult = this.recognizeColorWithPython(imagePath);
                
                if (pythonColorResult.success && pythonColorResult.data) {
                    console.log('✅ 使用Python颜色识别结果');
                    primaryColor = pythonColorResult.data;
                    dominantColors = [primaryColor];
                } else {
                    console.log('⚠️ Python颜色识别失败，使用JavaScript版本');
                    dominantColors = this.analyzeImageColors(clothingOnlyBuffer);
                    primaryColor = this.identifyPrimaryColor(dominantColors);
                }
            } catch (colorError) {
                console.error('颜色分析失败:', colorError);
                dominantColors = [{ rgb: [128, 128, 128], hex: '#808080', name: '灰色' }];
                primaryColor = { rgb: [128, 128, 128], hex: '#808080', name: '灰色' };
            }
            
            // 识别服装类型
            let type, style, pattern;
            try {
                type = this.identifyClothingType(clothingOnlyBuffer, dominantColors);
                style = this.identifyStyle(dominantColors, type);
                pattern = this.identifyPattern(clothingOnlyBuffer);
            } catch (typeError) {
                console.error('服装类型识别失败:', typeError);
                type = '其他';
                style = '休闲';
                pattern = '纯色';
            }
            ...
            let confidence;
            try {
                confidence = this.calculateConfidence(dominantColors, type);
            } catch (confidenceError) {
                console.error('置信度计算失败:', confidenceError);
                confidence = 0.5;
            }
            ...
            const result = {
                ...
                hasBackground: false,
                processing: {
                    backgroundSeparated: clothingOnlyBuffer !== imageBuffer,
                    colorsAnalyzed: dominantColors.length > 0,
                    typeIdentified: type !== '其他',
                    styleIdentified: style !== '休闲',
                    patternIdentified: pattern !== '纯色'
                }
            };
            ...
            console.log('增强服装识别结果:', result);
            return {
                success: true,
                data: result
            };
        } catch (error) {
            console.error('增强服装识别失败:', error);
            return {
                success: false,
                error: error.message || '服装识别失败',
                fallback: {
                    type: '其他',
                    color: { rgb: [128, 128, 128], hex: '#808080', name: '灰色' },
                    style: '休闲',
    ...
    async separateBackground(imageBuffer) {
        try {
            console.log('开始背景分离...');
            
            // 1. 分析图像亮度和颜色分布
            const brightness = this.calculateAverageBrightness(imageBuffer);
            const dominantColor = this.getDominantColor(imageBuffer);
            
            console.log(`图像亮度: ${brightness}, 主色调: ${JSON.stringify(dominantColor)}`);
            
            // 2. 创建服装区域掩码
            const clothingMask = this.createClothingMask(imageBuffer, dominantColor, brightness);
            
            // 3. 应用掩码提取服装区域
            const clothingOnlyBuffer = this.applyMask(imageBuffer, clothingMask);
            
            console.log('背景分离完成');
            return clothingOnlyBuffer;
        } catch (error) {
    ...
    calculateAverageBrightness(imageBuffer) {
        let totalBrightness = 0;
        let pixelCount = 0;
        
        for (let i = 0; i < imageBuffer.length - 2; i += 3) {
    ...
    getDominantColor(imageBuffer) {
        ...
        for (let i = 0; i < imageBuffer.length - 2; i += 3) {
            if (i + 2 < imageBuffer.length) {
                rTotal += imageBuffer[i];
                gTotal += imageBuffer[i + 1];
                bTotal += imageBuffer[i + 2];
                pixelCount++;
            }
        }
    ...
    createClothingMask(imageBuffer, dominantColor, brightness) {
        const mask = [];
        const threshold = this.calculateThreshold(brightness);
        ...
        for (let i = 0; i < imageBuffer.length - 2; i += 3) {
            if (i + 2 < imageBuffer.length) {
                ...
                // 计算与主色调的距离
                const distance = Math.abs(r - dominantColor.r) + 
                               Math.abs(g - dominantColor.g) + 
                               Math.abs(b - dominantColor.b);
                
                // 创建掩码：距离较小的像素更可能是服装
                const isClothing = distance < threshold;
                mask.push(isClothing);
            }
        }
    ...
    applyMask(imageBuffer, mask) {
        ...
        for (let i = 0; i < result.length - 2; i += 3) {
            if (maskIndex < mask.length) {
                if (!mask[maskIndex]) {
                    // 非服装区域设置为透明（如果是RGBA格式）
                    // 对于RGB格式，我们设置为接近背景的颜色
                    result[i] = 240;   // R
                    result[i + 1] = 240; // G
                    result[i + 2] = 240; // B
                }
        ...
        try {
            ...
            for (let i = 0; i < imageBuffer.length - 2; i += step * 3) {
                if (i + 2 < imageBuffer.length) {
                    ...
                    // 过滤掉背景像素（接近240,240,240的像素）
                    const isBackground = Math.abs(r - 240) < 20 && 
                                        Math.abs(g - 240) < 20 && 
                                        Math.abs(b - 240) < 20;
            ...
            console.log(`分析像素: 总像素=${pixelCount}, 服装像素=${clothingPixelCount}`);
            
            if (clothingPixelCount > 0) {
                const avgR = Math.round(rTotal / clothingPixelCount);
                const avgG = Math.round(gTotal / clothingPixelCount);
                const avgB = Math.round(bTotal / clothingPixelCount);
                
                colors.push({ r: avgR, g: avgG, b: avgB, percentage: 100 });
            } else if (pixelCount > 0) {
                // 如果没有检测到服装像素，使用所有像素的平均值
                ...
                colors.push({ r: 128, g: 128, b: 128, percentage: 100 });
            }
    ...
    identifyPrimaryColor(colors) {
        if (!colors || colors.length === 0) {
            return { rgb: [128, 128, 128], hex: '#808080', name: '灰色' };
        }
    ...
    recognizeColorWithPython(imagePath) {
        try {
            console.log('🐍 开始使用Python进行颜色识别...');
            
            // 构建Python脚本路径
            const pythonScript = path.join(__dirname, '..', 'services', 'python_color_recognizer.py');
            ...
            console.error('Python颜色识别失败:', error);
            return { success: false, error: error.message };
        }
    }
}

module.exports = new EnhancedClothingRecognitionService();